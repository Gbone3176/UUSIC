...
def np2th(weights, conv: bool = False):
    ...
    # Already a torch Tensor -> just permute if needed and return
    if isinstance(weights, torch.Tensor):
        if conv:
            # expecting HWIO -> OIHW; for torch tensor use permute
            if weights.ndim != 4:
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False):
        ...
        self.cls_head_2 = nn.Linear(config.hidden_size, 2)
        self.cls_head_4 = nn.Linear(config.hidden_size, 4)
    ...
    def forward(self, x):
        ...
        # === 分类分支：对 token 做全局平均池化 ===
        x_feat = x_tokens.mean(dim=1)  # (B, hidden)
        x_cls_2 = self.cls_head_2(x_feat)  # (B, 2)
        x_cls_4 = self.cls_head_4(x_feat)  # (B, 4)

        # === 分割分支 ===
        x_dec = self.decoder(x_tokens, features)   # (B, C, H, W)
        x_seg = self.segmentation_head(x_dec)      # (B, n_classes, H, W)
    ...
    def load_from(self, weights):
        with torch.no_grad():
            # ===== 统计计数器（不改变原逻辑）=====
            total_params = 0            # 参与 copy_ 的参数数
            resized_params = 0          # 发生了插值/重排尺寸的参数数（仅位置编码）
            encoder_units_called = 0    # 调用了多少个 encoder unit 的 load_from
            hybrid_units_called = 0     # 调用了多少个 hybrid unit 的 load_from
