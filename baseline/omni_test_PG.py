...
from networks.omni_vision_transformer_PG import OmniVisionTransformer as ViT_omni
...
def inference(args, model, test_save_path=None):
    ...
    if int(os.environ["LOCAL_RANK"]) == 0:
        ...
        for k, v in private_performence_seg.items():
            logging.info(f"{k}: {v}")
        mean_value = np.mean(list(private_performence_seg.values()))
        logging.info(f"Mean Dice: {mean_value:.4f}")

        logging.info("Private classification performance: ")
        for k, v in private_performence_cls.items():
            logging.info(f"{k}: {v}")
        mean_value = np.mean(list(private_performence_cls.values()))
        logging.info(f"Mean Acc: {mean_value:.4f}")
...
if __name__ == "__main__":
    ...
    if snapshot is None:
        raise ValueError("--resume argument is required but not provided")
    if not os.path.exists(snapshot):
        raise FileNotFoundError(f"Resume checkpoint file not found: {snapshot}")
