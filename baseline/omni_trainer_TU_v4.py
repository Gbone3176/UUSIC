...
def freeze_all_but_classifier_head(model: nn.Module, verbose: bool = True):
    """
    DDP/DP 兼容版：
    ...
    if hasattr(net, "early_classifier"):
        _freeze_module(net.early_classifier)
    else:
        raise AttributeError("model.early_classifier 不存在，检查你的模型定义。")
    ...
    def _locked_train(self, mode: bool = True):
        ...
        if hasattr(inner, "early_classifier"):  inner.early_classifier.eval()
        if hasattr(inner, "segmentation_head"):      inner.segmentation_head.eval()
        # 只有 early_classifier 跟随 mode
    ...
    if verbose:
        ...
        print(f"[Freeze] trainable params: {n_train:,} / {n_total:,} "
              f"({n_train/n_total*100:.4f}%) | only early_classifier is trainable.")
...
def omni_train(args, model, snapshot_path):
    ...
    classifier_head_params = freeze_all_but_classifier_head(model)
    ...
    optimizer = optim.AdamW(classifier_head_params, lr=base_lr, weight_decay=0.01, betas=(0.9, 0.999))
