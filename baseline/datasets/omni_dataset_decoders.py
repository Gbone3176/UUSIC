...
class USdatasetOmni_seg(Dataset):
    def __init__(self, base_dir, split, transform=None, prompt=False):
        ...
        print('='*20)
        cls_data_sets = os.listdir(os.path.join(base_dir, "segmentation"))
        print(f"Using private datasets: {cls_data_sets} for segmentation task.")
...
class USdatasetOmni_seg_decoders(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label_info = open(os.path.join(self.data_dir, "config.yaml")).readlines()
...
class USdatasetOmni_cls_decoders(Dataset):
    def __init__(self, base_dir, split, transform=None, prompt=False):
        self.transform = transform
        self.split = split
        self.data_dir = base_dir
    ...
    def __getitem__(self, idx):
        ...
        if not self.prompt:
            ...
            if dataset_name in available_type_prompt_list:
                random_number = random.random()
                mask_path = os.path.join(self.data_dir.replace("classification","segmentation"), "masks", img_name.split("/")[1])
                if random_number < 0.3:
                    sample = {'image': image/255.0, 'label': np.zeros(image.shape[:2])}
                    sample['type_prompt'] = type_prompt_one_hot_dict["whole"]
                elif random_number < 0.6:
