...
def omni_train(args, model, snapshot_path):
    ...
    weight_base = [
        0.5,  # for 10460 Thyroid(DDTI+TG3K+TNSCUI+TN3K)
        3,     # for 105
        1,     # for 452
        4,     # for 46
        4,     # for 84
    ...
    if args.resume is not None:
        # prediction = torch.load(args.resume, map_location='cpu')
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in tqdm(enumerate(trainloader_seg)):
            ...
            if progress >= 0.3:
                ...
                lr_ = base_lr * 0.33834  # 保持在30%进度时的学习率
    ...
    for epoch_num in iterator:
        if int(os.environ["LOCAL_RANK"]) == 0:
            ...
            for dataset_name in seg_val_set:
                ...
                val_loader = DataLoader(db_val, batch_size=batch_size, shuffle=False, num_workers=16, drop_last=False)
