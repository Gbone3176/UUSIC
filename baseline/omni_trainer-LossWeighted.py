...
class AdaptiveLossWeights(torch.nn.Module):
    ...
    def __init__(self, num_losses=2):
        ...
        # 初始化log(sigma^2)参数，对应每个损失的不确定性
        self.log_vars = torch.nn.Parameter(torch.zeros(num_losses))
    
    def forward(self, losses):
        """
        Args:
            losses: list of loss tensors [loss_ce, loss_dice]
        Returns:
            weighted_loss: 加权后的总损失
            weights: 当前权重值
        """
        weights = torch.exp(-self.log_vars)  # w_i = 1/sigma_i^2
        weighted_losses = [w * loss + 0.5 * log_var for w, loss, log_var in 
                          zip(weights, losses, self.log_vars)]
...
def omni_train(args, model, snapshot_path):
    ...
    # 初始化自适应损失权重
    adaptive_weights = AdaptiveLossWeights(num_losses=2).to(device)
    
    # 创建优化器，包含模型参数和权重参数
    model_params = list(model.parameters())
    weight_params = list(adaptive_weights.parameters())
    optimizer = optim.AdamW(model_params + weight_params, lr=base_lr, weight_decay=0.05, betas=(0.9, 0.999))
    ...
    if args.resume is not None:
        ...
        # 如果checkpoint中包含权重参数，也要加载
        if 'adaptive_weights' in checkpoint:
            adaptive_weights.load_state_dict(checkpoint['adaptive_weights'])
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in tqdm(enumerate(trainloader_seg)):
            ...
            # 使用自适应权重
            losses = [loss_ce, loss_dice]
            loss, current_weights = adaptive_weights(losses)
            ...
            # 记录当前权重
            writer.add_scalar('info/seg_weight_ce', current_weights[0].item(), seg_iter_num)
            writer.add_scalar('info/seg_weight_dice', current_weights[1].item(), seg_iter_num)

            logging.info('global iteration %d and seg iteration %d : loss : %f, CE weight: %.4f, Dice weight: %.4f' %
